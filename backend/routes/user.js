...
function input_validation(req, res, next) {
  try {
    ...
    if (!input_validation.success)
      return res.status(402).json({
        message: input_validation.error.message,
        success: false,
      });
    else {
      next();
...
router.put("/update", user_authentication, async (req, res) => {
  try {
    ...
    const is_new_password_valid = password_schema.safeParse(new_password)
      .success;
    ...
    const is_new_first_name_valid = first_name_schema.safeParse(
      new_first_name
    ).success;
    ...
    const is_new_last_name_valid = last_name_schema.safeParse(
      new_last_name
    ).success;
    ...
    const is_new_username_valid = username_schema.safeParse(new_username)
      .success;
...
router.get('/bulk',user_authentication, async (req, res) => {
 try {
  ...
  if (!filter) return res.status(402).json({
    message: 'filter is absent',
    success: false
  });
  const matched_users = await User.find({username: filter});
  return res.status(200).json({
    matched_users,
    success: true
  });
